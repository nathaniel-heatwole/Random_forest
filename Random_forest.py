...
other_vars = ['age recode', 'age missing', 'age estimated', 'male', 'sibsp', 'parch']
...
# age related
titanic['age recode'] = titanic['age'].apply(lambda a: 0 if np.isnan(a) else a)  # recodes missing age values to zero
...
rf_feature_imp = pd.DataFrame(importances, index=x.columns)
rf_feature_imp.rename(columns={0:'value'}, inplace=True)
